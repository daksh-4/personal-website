    ...
    (async function () {{
        ...
        async function fetchOld(sha) {{
            ...
            try {{
                ...
                cache[sha] = old ? old.innerHTML : '<p><em>Version unavailable.</em></p>';
            }} catch (e) {{
                cache[sha] = '<p><em>Version unavailable.</em></p>';
            }}
        }}
        ...
        async function showContent(pos) {{
            ...
            if (pos === versions.length) {{
                ...
                if (diffToggle.checked) {{
                    const ops = computeDiff(getPlainText(cache[sha]), getPlainText(currentHTML));
                    contentEl.innerHTML = ops ? renderDiff(ops) : cache[sha];
                }} else {{
                    contentEl.innerHTML = cache[sha];
                }}
            }}
            activePos = pos;
